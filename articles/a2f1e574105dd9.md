...
ゲームアプリのような場合、ウィジェットではなくグラフィックで画面を描画する必要があります。
グラフィック描画について記載します。

グラフィック描画を行うためのクラスとして`CustomPaint`が用意されています。
`CustomPaint`の引数に`CustomPainter`を継承したクラスを用意することで、独自の描画を行うことができます。
...
void paint(Canvas canvas, Size size) {
  ...
  p.style = PaintingStyle.fill;
  p.color = const Color.fromARGB(150, 0, 200, 255);
  // Offset(横位置, 縦位置), 半径, paint
  canvas.drawCircle(const Offset(100, 100), 30, p);
}
```
...
void paint(Canvas canvas, Size size) {
  ...
  p.style = PaintingStyle.fill;
  p.color = const Color.fromARGB(150, 0, 200, 255);
  Rect r = const Rect.fromLTWH(50, 100, 150, 200);
  canvas.drawOval(r, p);
}
```
